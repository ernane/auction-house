...
from src.adapters.exceptions import ExternalError
from src.domain.value_objects.price import CurrencyOption, Price
from src.drivers.rest.dependencies import get_submit_bid_use_case
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            AuctionNotActiveError(auction_id),
            status.HTTP_422_UNPROCESSABLE_ENTITY,
        ),
        (
            LowBidError(Price(value=10, currency=CurrencyOption.real)),
            status.HTTP_422_UNPROCESSABLE_ENTITY,
        ),
    ],
)
...
async def test_submit_bid_success(async_client: AsyncClient):
    ...
    app.dependency_overrides[get_submit_bid_use_case] = MockUseCase
    await async_client.post(url, json=payload)
